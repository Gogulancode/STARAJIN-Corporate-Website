...
async function main() {
  ...
  const headerMenuItems = [
    { en: 'Home', ko: '홈', route: '/' },
    { en: 'About', ko: '회사 소개', route: '/about' },
    { en: 'Services', ko: '서비스', route: '/services' },
    { en: 'Projects', ko: '프로젝트', route: '/projects' },
    { en: 'Contact', ko: '연락처', route: '/contact' }
  ];
  ...
  for (let i = 0; i < headerMenuItems.length; i++) {
    ...
    const menuItem = await prisma.menuItem.create({
      data: {
        ...
        translations: {
          create: [
            { locale: 'en', label: item.en },
            { locale: 'ko', label: item.ko }
          ]
        }
      }
    });
  ...
  // Create footer menu items with children
  const companyMenuItem = await prisma.menuItem.create({
    data: {
  ...
  const companySubItems = [
    { en: 'About Us', ko: '회사 소개', route: '/about' },
    { en: 'Our Team', ko: '팀 소개', route: '/team' },
    { en: 'Careers', ko: '채용 정보', route: '/careers' }
  ];

  for (let i = 0; i < companySubItems.length; i++) {
    const item = companySubItems[i];
    await prisma.menuItem.create({
      data: {
        menuId: footerMenu.id,
        parentId: companyMenuItem.id,
        linkType: 'internal',
        route: item.route,
        newTab: false,
        isEnabled: true,
